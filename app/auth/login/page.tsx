...
import { signIn, getGoogleAuthUrl } from '@/lib/auth'
...
function LoginForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      const dest = returnUrl && !returnUrl.startsWith('/admin') ? returnUrl : result.redirectTo
      router.push(dest)
      router.refresh()
    })
  }
    ...
    <div className="w-full max-w-md">
      <div className="bg-card border border-border rounded-2xl p-8 shadow-2xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-sm">Sign in to your Bella Crosta account</p>
        </div>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center">
            <span className="px-3 bg-card text-xs text-muted-foreground">Don&apos;t have an account?</span>
          </div>
        </div>
      ...
      <p className="text-center text-xs text-muted-foreground mt-5">
        Restaurant staff?{' '}
        <span className="text-primary">Sign in with your admin credentials to access the dashboard.</span>
      </p>
    </div>
  )
}
